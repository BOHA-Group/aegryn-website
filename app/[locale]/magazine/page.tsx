...
import { getAdminUser, hasAdminTokenCookie, hasAdminProofCookie } from '@/lib/adminAuth'
...
export default async function MagazineHubPage({ params }: Props) {
  ...
  const [tHub, tSub, flags, adminUser, adminTokenOk, adminProofOk] = await Promise.all([
  ...
  /* Boutons "Explorer en ligne"/"Feuilleter le PDF" débloqués visuellement :
     ...
     cookie token ou preuve signée) — cohérent avec le gate réel appliqué dans
     canAccessIssue() (lib/magazineAccess.ts). */
  const isPreview = process.env.VERCEL_ENV !== 'production' || !!adminUser || adminTokenOk || adminProofOk
