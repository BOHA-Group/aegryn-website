...
export default async function RoadmapPage({ params }: Props) {
  ...
  const breadcrumbLd = breadcrumbJsonLd([
    ...
    { name: t('meta.title').split(' — ')[0], url: `https://aegryn.com/${locale}/roadmap` },
  ])

  const statusColor = (s: string) =>
    s.toLowerCase().includes('développement') ||
    s.toLowerCase().includes('development') ||
    s.toLowerCase().includes('sviluppo') ||
    s.toLowerCase().includes('entwicklung') ||
    s.toLowerCase().includes('ontwikkeling')
    ...
    <main id="main">
      ...
      <section className="bg-ag-navy pt-32 pb-24 px-6">
        <div className="max-w-7xl mx-auto">
          <p className="font-mono text-[10px] tracking-[0.28em] uppercase text-white/40 mb-6">
            {t('label')}
          </p>
          <h1
          ...
          <p className="font-sans text-[15px] text-white/55 leading-relaxed max-w-lg">
            {t('desc')}
          </p>
        </div>
      </section>
            ...
            {items.map((item, i) => (
              <div key={i} className="bg-ag-white p-8 md:p-12 flex flex-col md:flex-row gap-8 md:gap-16">
                <div className="shrink-0 w-28">
                  <p className="font-mono text-[10px] tracking-[0.22em] uppercase text-ag-gray-light mb-1">
                    {item.phase}
                  </p>
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-semibold rounded-sm ${statusColor(item.status)}`}>
                    <Clock size={10} />
                    {item.status}
                  </span>
                </div>
