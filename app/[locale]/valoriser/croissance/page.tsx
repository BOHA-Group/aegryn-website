...
import { ArrowUpRight, FileWarning, UserCog, ServerCog, LineChart, Building2, ShieldCheck, Cpu, Users2, UserSearch } from 'lucide-react'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'valoriser.croissance.meta' })
  return generateAegrynMetadata({ title: t('title'), description: t('desc'), path: '/valoriser/croissance', locale })
}

export default async function ValoriserCroissancePage({ params }: Props) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'valoriser.croissance' })

  const PAINS = [
    { key: 'pain1', icon: <FileWarning size={18} className="text-ag-apex-ink" /> },
    { key: 'pain2', icon: <UserCog size={18} className="text-ag-apex-ink" /> },
    { key: 'pain3', icon: <ServerCog size={18} className="text-ag-apex-ink" /> },
    { key: 'pain4', icon: <LineChart size={18} className="text-ag-apex-ink" /> },
  ] as const

  const SERVICES = [
    { key: 'svc1', icon: <Building2 size={18} className="text-ag-apex-ink" /> },
    { key: 'svc2', icon: <ShieldCheck size={18} className="text-ag-apex-ink" /> },
    { key: 'svc3', icon: <Cpu size={18} className="text-ag-apex-ink" /> },
    { key: 'svc4', icon: <Users2 size={18} className="text-ag-apex-ink" /> },
    { key: 'svc5', icon: <UserSearch size={18} className="text-ag-apex-ink" /> },
  ] as const
  ...
  return (
    <main className="bg-ag-white">
      ...
      <section className="py-24 px-6 border-t border-ag-border">
        <div className="max-w-7xl mx-auto">
          <p className="font-mono text-[10px] tracking-[0.28em] uppercase text-ag-gray-light mb-12">{t('cifsoLabel')}</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-ag-border border border-ag-border">
            <div className="bg-ag-white p-10 flex flex-col gap-3">
              <p className="font-sans font-bold text-ag-black text-[17px] leading-snug tracking-[-0.02em]">{t('cifso1Title')}</p>
              <p className="font-sans text-[13px] text-ag-gray leading-relaxed">{t('cifso1Desc')}</p>
            </div>
            <div className="bg-ag-white p-10 flex flex-col gap-3">
              <p className="font-sans font-bold text-ag-black text-[17px] leading-snug tracking-[-0.02em]">{t('cifso2Title')}</p>
              <p className="font-sans text-[13px] text-ag-gray leading-relaxed">{t('cifso2Desc')}</p>
            </div>
            <div className="bg-ag-white p-10 flex flex-col gap-3">
              <p className="font-sans font-bold text-ag-black text-[17px] leading-snug tracking-[-0.02em]">{t('cifso3Title')}</p>
              <p className="font-sans text-[13px] text-ag-gray leading-relaxed">{t('cifso3Desc')}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-ag-border border border-ag-border">
            <div className="bg-ag-white p-10 flex flex-col gap-3">
              ...
              <p className="font-sans text-[14px] text-ag-gray leading-relaxed">{t('audience1Desc')}</p>
            </div>
            <div className="bg-ag-white p-10 flex flex-col gap-3">
              <p className="font-sans font-semibold text-ag-black text-[16px] leading-snug tracking-[-0.02em]">{t('audience2Title')}</p>
              <p className="font-sans text-[14px] text-ag-gray leading-relaxed">{t('audience2Desc')}</p>
            </div>
          </div>
        </div>
      </section>
