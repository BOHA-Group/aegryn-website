...
import { ArrowUpRight, ShieldAlert, Bug, Scale, Users2, Lightbulb, Cpu, Hammer, ScrollText, UserSearch } from 'lucide-react'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'valoriser.lancement.meta' })
  return generateAegrynMetadata({ title: t('title'), description: t('desc'), path: '/valoriser/lancement', locale })
}

export default async function ValoriserLancementPage({ params }: Props) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'valoriser.lancement' })

  const PAINS = [
    { key: 'pain1', icon: <ShieldAlert size={18} className="text-ag-apex-ink" /> },
    { key: 'pain2', icon: <Bug size={18} className="text-ag-apex-ink" /> },
    { key: 'pain3', icon: <Scale size={18} className="text-ag-apex-ink" /> },
    { key: 'pain4', icon: <Users2 size={18} className="text-ag-apex-ink" /> },
  ] as const

  const SERVICES = [
    { key: 'svc1', icon: <Lightbulb size={18} className="text-ag-apex-ink" /> },
    { key: 'svc2', icon: <Cpu size={18} className="text-ag-apex-ink" /> },
    { key: 'svc3', icon: <Hammer size={18} className="text-ag-apex-ink" /> },
    { key: 'svc4', icon: <ScrollText size={18} className="text-ag-apex-ink" /> },
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-ag-border border border-ag-border">
            <div className="bg-ag-white p-10 flex flex-col gap-3">
              <p className="font-sans font-bold text-ag-black text-[18px] leading-snug tracking-[-0.02em]">{t('cifsoIndexTitle')}</p>
              <p className="font-sans text-[14px] text-ag-gray leading-relaxed">{t('cifsoIndexDesc')}</p>
            </div>
            <div className="bg-ag-white p-10 flex flex-col gap-3">
              <p className="font-sans font-bold text-ag-black text-[18px] leading-snug tracking-[-0.02em]">{t('cifsoGradeTitle')}</p>
              <p className="font-sans text-[14px] text-ag-gray leading-relaxed">{t('cifsoGradeDesc')}</p>
            </div>
          </div>
        </div>
      </section>
