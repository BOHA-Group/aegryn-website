...
import { ArrowUpRight, ShieldAlert, TrendingDown, Scissors, HeartCrack, Target, Wrench, UserCog2, Scale } from 'lucide-react'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'valoriser.restructuration.meta' })
  return generateAegrynMetadata({ title: t('title'), description: t('desc'), path: '/valoriser/restructuration', locale })
}

export default async function ValoriserRestructurationPage({ params }: Props) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'valoriser.restructuration' })

  const PAINS = [
    { key: 'pain1', icon: <ShieldAlert size={18} className="text-ag-apex-ink" /> },
    { key: 'pain2', icon: <TrendingDown size={18} className="text-ag-apex-ink" /> },
    { key: 'pain3', icon: <Scissors size={18} className="text-ag-apex-ink" /> },
    { key: 'pain4', icon: <HeartCrack size={18} className="text-ag-apex-ink" /> },
  ] as const

  const SERVICES = [
    { key: 'svc1', icon: <Target size={18} className="text-ag-apex-ink" /> },
    { key: 'svc2', icon: <Wrench size={18} className="text-ag-apex-ink" /> },
    { key: 'svc3', icon: <UserCog2 size={18} className="text-ag-apex-ink" /> },
    { key: 'svc4', icon: <Scale size={18} className="text-ag-apex-ink" /> },
  ] as const
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-ag-border border border-ag-border">
            <div className="bg-ag-white p-10 flex flex-col gap-3">
              <p className="font-sans font-bold text-ag-black text-[18px] leading-snug tracking-[-0.02em]">{t('cifso1Title')}</p>
              <p className="font-sans text-[14px] text-ag-gray leading-relaxed">{t('cifso1Desc')}</p>
            </div>
            <div className="bg-ag-white p-10 flex flex-col gap-3">
              <p className="font-sans font-bold text-ag-black text-[18px] leading-snug tracking-[-0.02em]">{t('cifso2Title')}</p>
              <p className="font-sans text-[14px] text-ag-gray leading-relaxed">{t('cifso2Desc')}</p>
            </div>
          </div>
        </div>
      </section>
