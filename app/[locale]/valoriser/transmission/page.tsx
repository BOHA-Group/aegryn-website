...
import { ArrowUpRight, UserX, FolderX, ClipboardX, Users2, ScrollText, ClipboardList, Handshake, BadgeCheck } from 'lucide-react'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'valoriser.transmission.meta' })
  return generateAegrynMetadata({ title: t('title'), description: t('desc'), path: '/valoriser/transmission', locale })
}

export default async function ValoriserTransmissionPage({ params }: Props) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'valoriser.transmission' })

  const PAINS = [
    { key: 'pain1', icon: <UserX size={18} className="text-ag-apex-ink" /> },
    { key: 'pain2', icon: <FolderX size={18} className="text-ag-apex-ink" /> },
    { key: 'pain3', icon: <ClipboardX size={18} className="text-ag-apex-ink" /> },
  ] as const

  const SERVICES = [
    { key: 'svc1', icon: <Users2 size={18} className="text-ag-apex-ink" /> },
    { key: 'svc2', icon: <ScrollText size={18} className="text-ag-apex-ink" /> },
    { key: 'svc3', icon: <ClipboardList size={18} className="text-ag-apex-ink" /> },
    { key: 'svc4', icon: <Handshake size={18} className="text-ag-apex-ink" /> },
  ...
  return (
    <main className="bg-ag-white">
      ...
      {/* ── Ce que les négociations révèlent trop tard ── */}
