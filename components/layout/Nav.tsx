...
function ThinkingMegaMenu({ t, onClose }: { t: ReturnType<typeof useTranslations>; onClose: () => void }) {
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -4 }}
      transition={{ duration: 0.2, ease: [0.16, 1, 0.3, 1] }}
      className="absolute top-full left-0 mt-2 w-[640px] bg-ag-white border border-ag-border shadow-xl rounded-2xl overflow-hidden z-50"
    >
  ...
  return (
    ...
    <header ref={navRef} className="fixed top-0 left-0 right-0 z-50 bg-ag-white border-b border-ag-border">
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between h-16">
        ...
        {/* Mobile toggle */}
        <button
          className="lg:hidden rounded-lg p-2 text-ag-gray hover:text-ag-black transition-colors"
