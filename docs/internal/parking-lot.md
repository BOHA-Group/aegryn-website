...
- Limite connue : aucun flux officiel ouvert ne publie de multiples M&A privés ; ces séries restent issues des données curées Aegryn (`cifso_market_multiples`, `benchmark_data`) et des dossiers certifiés. Pour brancher un fournisseur de multiples sous licence (FactSet, PitchBook, Dealroom), ajouter un connecteur dans `CONNECTORS` avec sa clé en variable d'environnement : aucun autre changement requis.
