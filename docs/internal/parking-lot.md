...
- **Périmètre exact de l'alimentation de l'Index par les contributions anonymes (`cifso_index_contributed_data`), vérifié dans le code — trois nuances à connaître avant de communiquer sur ce mécanisme :**
  1. **Sauvegarde pérenne : oui.** Chaque soumission de l'étape 1 du parcours `/valuation/index` écrit une seule ligne anonymisée (jamais réécrite « pour le test » puis « pour l'abonné » séparément — c'est le même enregistrement qui sert aux deux usages), avec garantie de retry (`lib/contributeQueue.ts`) en cas de panne réseau/serveur transitoire.
  2. **Pas d'injection en temps réel dans l'aperçu « Index non abonné ».** `IndexTestView.tsx` (étape 2) relit `/api/valuation/index` → `getIndexSnapshot()` → `cifso_index_benchmarks`, un instantané déjà calculé par le refresh hebdomadaire — jamais `cifso_index_contributed_data` en direct. L'utilisateur voit donc le tableau agrégé *existant* (antérieur à sa propre soumission), avec sa position personnelle simplement superposée en surbrillance (industrie/grade/score calculés côté client à l'étape 1) — sa donnée n'est pas mathématiquement fondue dans les pourcentages affichés pendant sa propre session.
  3. **Injection systématique côté « Index abonné », mais avec délai et périmètre limités.** Le refresh hebdomadaire (`lib/cifsoIndexRefresh.ts`, tous les lundis) fusionne bien les scores CIFSO contribués avec les dossiers certifiés publiés pour les percentiles par dimension C/I/F/S/O, et bascule sur les contributions pour les métriques par vertical (croissance, NRR, marge brute) une fois 10 échantillons atteints par vertical (remplaçant alors l'estimation curée `benchmark_data`). Mais **les multiples EV/Revenue et EV/EBITDA par industrie** (l'essentiel de l'onglet « Multiples de marché », issus de `cifso_market_multiples`) **restent purement curés manuellement** — les contributions n'y touchent jamais aujourd'hui, volontairement, une auto-estimation n'étant pas une transaction réelle. Délai réel avant impact : jusqu'à 7 jours (prochain lundi), jamais immédiat. À noter aussi : les métriques par vertical (croissance/NRR/marge) que le refresh calcule déjà à partir des contributions ne sont pour l'instant affichées nulle part dans `IndexTestView.tsx` (qui ne montre que le multiple ARR et l'EV/EBITDA) — à ajouter si on veut rendre ce mécanisme visible aux utilisateurs.
  4. **Pistes ouvertes, non décidées :** (a) refléter la contribution de l'utilisateur immédiatement dans son propre aperçu de test (nécessiterait un calcul côté client ou un endpoint dédié, distinct de l'instantané hebdomadaire partagé) ; (b) étendre l'influence des contributions aux multiples EV/Revenue eux-mêmes, avec les mêmes précautions de fiabilité déjà discutées pour les sources externes (seuil d'échantillon, jamais une contribution isolée, distinction claire entre donnée déclarative et transaction vérifiée).
